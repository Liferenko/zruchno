...
### D4: Search revealed by pull-up, field at bottom, results bottom-anchored
The right-hand screen's reversed list uses a `NestedScrollConnection.onPostScroll` so that a negative `available.y` (finger dragging up, which is one direction beyond the reversed list's natural bottom overscroll) accumulates past a threshold (~64.dp) while search is hidden and reveals the shared, transparent, monochrome search field pinned at the **bottom** of the `Column` (thumb-reach, alongside the list). The list keeps `reverseLayout = true` in every state, so search results run bottom-up exactly like the normal list; `imePadding()` at the root keeps the field and the bottom-anchored first result above the soft keyboard.

- **Why:** Material3 `PullToRefreshBox` is pull-DOWN-only; a bottom-up reveal inverts it for thumb grammar. With `reverseLayout`, the finger-up drag yields the unconsumed `dy` at the list end, matching the "pulled down-to-up" request. A uniform bottom-up anchoring (also while searching) keeps interaction grammar identical to the normal list — the IME overlap is solved by `imePadding` at the root rather than by switching anchoring.
- **Alternative (tried):** switching to a top-anchored list while searching (`key(searchActive)` + `reverseLayout = !searchActive`), so a single match appeared above the keyboard — rejected on device feedback: results must run bottom-up like the non-hidden apps.
- **Risk:** gesture-nav interference on the physical device → mitigation: the pull zone starts above the navigation inset and ×/back remain available; validation flagged as a manual task.
