...
- [x] 3.3 Add pull-up search detection (~64dp threshold) via `NestedScrollConnection` onPostScroll that reveals `AppSearchField` pinned at the bottom of the screen; the list stays `reverseLayout = true` while searching so results run bottom-up like the rest; `imePadding()` keeps content above the keyboard; clear/×/back collapse and restore the full list
...
- [x] 6.2 Manual (emulator): corner toggle enters/exits; right-hand list shows hidden apps above visible (bottom-anchored); hiding an app on one screen reflects on the other
- [x] 6.3 Manual (emulator): pull-up in the list reveals search at the bottom, typing filters the list bottom-up (same as the main list), ×/back restores the full list
- [x]  6.4 Manual (emulator): corner toggle is tappable and not blocked by the system bars; pull-up drag does not trigger the system gesture
- [x] 6.5 Manual (device, Mi Mix Flip 3): right-hand list within thumb reach; list geometry (full-screen) and pull threshold feel right; tuning documented